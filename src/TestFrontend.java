...
public class TestFrontend {
	...
	public void runTests() {
		...
		System.out.print("Test 'r' load ratings selection screen (WARNING: if pressing 'x' twice from the ratings selection screen does not exit app, test won't exit and will run indefinitely!): ");
		if (this.testFrontendRForRatings()) {
		    System.out.println("PASSED");
		} else {
		    System.out.println("FAILED");
		}
		System.out.print("Test genre selection screen does not list movies (WARNING: if pressing 'x' twice from the genre selection screen does not exit app, test won't exit and run indefinitely!): ");
		if (this.testFrontendGenresNoMovie()) {
		    System.out.println("PASSED");
		} else {
		    System.out.println("FAILED");
		}	
	 ...
	 * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
	 * The test succeeds if the ratings selection screen contains options for ratings
	 * ranging from 0-10. It fails if the ratings screen does not contain "0-10", the
	 * front end has not been instantiated (is null), or there is an exception.
	 * @return true if the test passed, false if it failed
	 */
	    ...
	    InputStream standardIn = System.in;
	    try {
		// set the input stream to our input (with an r to test that the program lists ratings)
		String input = "r" + System.lineSeparator() + "x" + System.lineSeparator() + "x";
		InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
		System.setIn(inputStreamSimulator);
		ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
		// set the output to the steam captor to read the output of the front end
		System.setOut(new PrintStream(outputStreamCaptor));
		// instantiate when front end is implemented
		Object frontend = new Frontend();
			((Frontend)frontend).run(new Backend(new StringReader(
			...
			// add all tests to this method
			String appOutput = outputStreamCaptor.toString();
			if (frontend != null && appOutput.contains("0-10")) {
				// test passes if all possible rating options are shown
				return true;
			} else {
				// test failed
				return false;
	 ...
	 * passes in 'g' as a command to go to the genre selection mode. It then exists
	 * the app by pressing 'x' to go back to the main mode and another 'x' to exit.
	 * The test succeeds if the genre selection screen does not contain any selected
	 * movies. The genre selection screen should start out with no genres selected, 
	 * so it should not start out displaying movies. It fails if the genre selection 
	 * screen displays movies by default, the front end has not been instantiated 
	 * (is null), or there is an exception.
	 * @return true if the test passed, false if it failed
	 */
	public boolean testFrontendGenresNoMovie() {
		PrintStream standardOut = System.out;
		InputStream standardIn = System.in;
		try {
			// set the input stream to our input (with an g to test of the program lists genres)
